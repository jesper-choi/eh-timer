...
**https://jesper-choi.github.io/eh-timer/**

태블릿 크롬으로 위 주소를 열고 **메뉴 → 홈 화면에 추가**. 큐브 아이콘이 생기고, 주소창 없는 전체화면 앱으로 뜬다.
한 번 열어두면 서비스 워커가 앱 전체를 캐시하므로 **PC가 꺼져 있어도, 인터넷이 없어도(비행기모드) 그대로 동작**한다.

- 조작은 터치로 동일 — 가운데를 꾹 눌렀다 떼면 시작, 아무 데나 탭하면 정지
- 솔브 중 화면이 꺼지지 않는다 (wake lock)
- 기록은 **태블릿 브라우저에 저장**된다. 이 주소에는 gist 토큰이 없으므로 PC와 자동 동기화는 하지 않는다.
  옮기려면 `내보내기`로 JSON을 받아 PC 쪽에서 `가져오기` (덮어쓰지 않고 병합된다)
- 코드를 고친 뒤 `git push` 하면 Pages가 다시 빌드되고, 태블릿은 다음에 온라인으로 열 때 갱신된다

같은 와이파이에서 PC 서버에 붙는 방법(기록이 PC·gist와 자동 공유됨)은 아래 "LAN 접속" 참고.

### LAN 접속 (PC를 켠 채로 쓸 때)

`.env`에 `EH_TIMER_LAN=1` 을 추가하거나 `node --env-file=.env server.js 8000 --lan` 으로 실행하면
접속 주소를 출력한다 (`태블릿/폰에서: http://192.168.0.12:8000`). 이 경우 기록이 PC 파일·gist에 함께 쌓인다.

> 주의: LAN 개방은 같은 와이파이의 누구나 접속·수정할 수 있다(인증 없음). 집 와이파이에서만 쓸 것.
> 또한 http라서 오프라인 캐시(서비스 워커)는 동작하지 않는다.
